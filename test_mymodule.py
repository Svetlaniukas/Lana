...
def test_empty_file_shut_return_empty_dictionary():
    dict_empty = mymodule.open_staff_file("testing/staff_empty_file.txt", ':')
...
def test_delimiter_comma_shut_retur_valid_dictionary():
    persons = mymodule.open_staff_file("testing/staff_name_surname.txt", ',')
...
def test_delimiter_slash_shut_retur_valid_dictionary():
    valid_dictionary = mymodule.open_staff_file("testing/staff_position.txt", '/')
    assert valid_dictionary['Tomas'] == 'web developer'


def test_not_valid_delimiter_shud_trow_error():
    dict = mymodule.open_staff_file("testing/staff_position.txt", ':::')
    assert len(dict) == 0

def test_3_line_file_shut_retur_3_itiems():
    line_test = mymodule.open_staff_file("testing/staff_name_surname.txt", ',')
...
def test_1_line_file_shut_retur_1_itiems():
    test_1_line = len(mymodule.open_staff_file("testing/staff_position.txt", '/'))
